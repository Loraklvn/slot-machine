...
> {
  static get defaultProps() {
    return {
      ...
      styles: {},
      renderTextContent: (currentChar: any) => currentChar,
  ...
  componentDidMount() {
    const { delay, initialAnimation } = this.props as Record<string, any>;
    if (!initialAnimation) {
      return;
    }
  ...
  getAdjustedAnimationValues(props: Record<string, any>) {
  ...
  getPosition(index: number, props = this.props) {
    const position = -1 * index * props.height;
    return position;
  }

  getAlignedValues(props: Record<string, any>) {
  ...
  getInitialSlotsValues(props: Record<string, any>) {
    const values = [];
  ...
  startInitialAnimation() {
    ...
    const animations = values.map(
      (value: Animated.Value | Animated.ValueXY, i: number) => {
  ...
  renderContent(currentChar: any, i: any, range: any) {
  ...
  renderSlot(charToShow: any, position: React.Key) {
    const {
      range,
    ...
    const { initialAnimation, values } = this.state as Record<string, any>;
    const charToShowIndex = range.indexOf(charToShow);

    const slots = range
      .split("")
      .map((num: any, i: React.Key | null | undefined) => {
        let currentChar = num;
        if (initialAnimation) {
